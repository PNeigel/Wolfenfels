...
array<GLfloat, 8> TextureAtlas::getUVcoords(glm::vec2 & index)
{
	...
	GLfloat xMin = (index.x * singleTexWidth) + 0.5 * PIXEL_WIDTH;
	GLfloat xMax = (xMin + singleTexWidth) - PIXEL_WIDTH;

	GLfloat yMax = (texPerCol - index.y) * singleTexHeight - 0.5 * PIXEL_HEIGHT;
	...
	assert(xMin >= 0.5 * PIXEL_WIDTH);
	assert(xMax <= 1 - 0.5 * PIXEL_WIDTH);
	assert(yMin >= 0.5 * PIXEL_HEIGHT);
	assert(yMax <= 1 - 0.5 * PIXEL_HEIGHT);
